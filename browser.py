...
def _open_context_with_heal(p, profile_dir, headless):
    ...
    # Pre-launch: free a stale holder of THIS profile (a leftover Edge from a prior
    # op that didn't close cleanly → "ProcessSingleton: profile already in use").
    # Safe: the profile is dedicated and browser ops are serialized by browser_lock.
    try:
        from browser_cleanup import kill_profile_processes, remove_stale_locks

        if kill_profile_processes(profile_dir):
            time.sleep(1.5)  # let the OS release the lock after killing the holder
