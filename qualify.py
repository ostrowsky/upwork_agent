...
QUALIFY_SYSTEM = (
    ...
    "2) БЮДЖЕТ. На Upwork БОЛЬШИНСТВО хороших вакансий — почасовые без указанной общей суммы; "
    "это НОРМА, НЕ повод для SKIP. Для Hourly: если ставка адекватна (макс ≳ $20/час) и стек/фокус "
    "совпадают — APPLY (НЕ требуй указанного общего бюджета и НЕ занижай fit из-за отсутствия суммы). "
    "SKIP по бюджету ТОЛЬКО если: (а) почасовая ставка мусорная (макс < $15/час), ИЛИ (б) объём явно "
    "крошечный («один экран/спрайт», разовая мелочь), ИЛИ (в) Fixed-price явно НИЖЕ минимума диапазона "
    "(напр. $15, $300 при минимуме $5k). Не сравнивай часовую ставку напрямую с проектным бюджетом.\n"
    "3) ИСКЛЮЧЕНИЯ из стратегии (например «без NFT/gambling/slots») соблюдай буквально.\n"
    "4) ФОКУС. Если стратегия задаёт приоритет (например «фокус: мультиплеер/Photon») — вакансии "
    "вне фокуса при совпадающем стеке оценивай ПОНИЖЕННЫМ fit (<55), а явно профильные (мультиплеер/"
    "Photon) — высоким. Если фокус сформулирован как требование («только …») — вне фокуса SKIP.\n"
    "Не «натягивай» вакансию на стратегию. fit_score — честное совпадение со стеком И фокусом; "
    "risk_score — риск. Для SKIP всегда дай skip_reason; summary — 1–2 предложения на языке вакансии, "
    "явно отметь стек, фокус и КАК бюджет/часовой объём соотносится с проектным диапазоном."
)
