...
- **Quality-gate синтетического кейса**: если LLM вернул «пустышку» (нет результатов/метрик,
  слишком короткое summary) — кейс НЕ сохраняется (`ok=False`, reason «too thin»), и отклик
  цитирует лучшие СУЩЕСТВУЮЩИЕ кейсы (`rank_cases`) вместо фабрикации пустого.
