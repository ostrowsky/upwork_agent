...
- **СТРОГАЯ квалификация (жёсткие правила, нарушение → SKIP):**
  ...
  - **Бюджет**: почасовые без общей суммы — НОРМА (не повод для SKIP). Hourly с адекватной ставкой
    (макс ≳ $20/ч) + совпадение стека/фокуса → APPLY. SKIP по бюджету только при мусорной ставке
    (< $15/ч), крошечном объёме, или Fixed-price явно ниже минимума ($15/$300 при минимуме $5k).
    Часовую ставку НЕ сравнивать напрямую с проектным бюджетом.
  - **Фокус**: если стратегия задаёт приоритет (напр. «мультиплеер/Photon») — вне-фокусные
    on-stack вакансии получают пониженный fit (<55); «только …» → SKIP вне фокуса.
  - Исключения стратегии (`без NFT/gambling/slots`) — буквально.
