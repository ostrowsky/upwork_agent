...
# Persist operational UI inputs across restarts (data/ui_settings.json).
_UI_SETTINGS_PATH = __import__("pathlib").Path(__file__).resolve().parent / "data" / "ui_settings.json"
_UI_KEYS = ("n_submit", "job_sort_by", "job_sort_dir", "nav_section_radio")
...
def _save_ui_settings():
    import json
    try:
        cur = {k: st.session_state[k] for k in _UI_KEYS if k in st.session_state}
        _UI_SETTINGS_PATH.parent.mkdir(parents=True, exist_ok=True)
        _UI_SETTINGS_PATH.write_text(json.dumps(cur), encoding="utf-8")
    except OSError:
        pass
...
def code_version():
    """(commit_hash, date) of the running code. git first, else newest source mtime."""
    import datetime
    import subprocess

    try:
        out = subprocess.run(
            ["git", "-C", str(_APP_DIR), "log", "-1", "--format=%h|%cd", "--date=format:%Y-%m-%d %H:%M"],
        ...
        pass
    try:
        newest = max(p.stat().st_mtime for p in _APP_DIR.glob("*.py"))
        return "local", datetime.datetime.fromtimestamp(newest).strftime("%Y-%m-%d %H:%M")
    except ValueError:
        return "?", ""


def _warn_if_stale_code():
    """Banner when source files changed after the process started (Streamlit caches
    imported modules → a rerun runs NEW app.py but OLD qualify/submit/etc.)."""
    import database

    here = __import__("pathlib").Path(__file__).resolve().parent
    try:
        newest = max(p.stat().st_mtime for p in here.glob("*.py"))
    except ValueError:
        return
    if newest > database.IMPORT_TIME + 2:  # 2s grace
        st.warning(
            "⚠️ Код на диске обновлён после запуска UI, но Streamlit держит **старые "
            "модули** (qualify/submit/…). Изменения НЕ применятся, пока не перезапустишь UI: "
            "**Ctrl+C в окне `run_ui.bat` → запустить заново**.",
            icon="⚠️",
        )
...
def generate_case_for_job_ui(job_id):
    ...
    try:
        ...
        # Re-draft the proposal so its proof cites a real case. On a successful
        # synthetic case it cites + attaches it; on a thin/failed one it falls back
        # to the best EXISTING ranked case (no fabricated empty case).
        from database import Proposal
        from proposals import generate_proposal

        if db.query(Proposal).filter(Proposal.job_id == job_id, Proposal.status == "DRAFT").first():
            try:
                gp = generate_proposal(job, task, db)
                res["redrafted"] = True
                res["cited_cases"] = gp.get("cases")
            except Exception:  # noqa: BLE001
                res["redrafted"] = False
        return res
    finally:
...
def render_dashboard():
    ...
    # Funnel drill-down — which exact jobs are behind the numbers (with links).
    with st.expander("🔎 Детализация воронки (конкретные отклики / исходы)"):
        from database import Proposal as _P

        _ddb = get_db_session()
        try:
            sent = (_ddb.query(Job)
                    .join(_P, _P.job_id == Job.id)
                    .filter(_P.status == "SENT")
                    .order_by(Job.id.desc()).all())
            st.markdown(f"**Отправленные отклики ({len(sent)}):**")
            for j in sent:
                p = _ddb.query(_P).filter(_P.job_id == j.id, _P.status == "SENT").first()
                oc = f" · исход: **{j.outcome}**" if j.outcome else ""
                link = f"[ссылка]({j.source_url})" if j.source_url else "—"
                st.markdown(f"- #{j.id} {(j.title or '')[:55]} · {p.connects_spent or '?'} connects{oc} · {link}")
            outs = _ddb.query(Job).filter(Job.outcome.isnot(None)).order_by(Job.id.desc()).all()
            if outs:
                st.markdown("**Исходы:**")
                for j in outs:
                    link = f"[ссылка]({j.source_url})" if j.source_url else "—"
                    st.markdown(f"- #{j.id} **{j.outcome}** — {(j.title or '')[:50]} · {link}"
                                + (f"\n    _разбор: {j.loss_reason[:120]}_" if j.loss_reason else ""))
        finally:
            _ddb.close()
...
def render_jobs():
    ...
    with col_f:
        status_filter = st.selectbox(
            "Фильтр по статусу",
            ["Все", "NEW", "READY_TO_PROPOSE", "PROPOSAL_DRAFTED", "SENT", "SKIPPED", "CLOSED", "ERROR"],
        )
    if active is None:
    ...
    # Sorting by fit / risk (asc/desc); default keeps newest-first.
    so1, so2 = st.columns([2, 1])
    sort_by = so1.selectbox("Сортировка", ["По дате (новые)", "Fit", "Risk"], key="job_sort_by")
    sort_desc = so2.selectbox("Порядок", ["По убыванию", "По возрастанию"], key="job_sort_dir") == "По убыванию"
    if sort_by in ("Fit", "Risk"):
        _key = (lambda j: j.fit_score) if sort_by == "Fit" else (lambda j: j.risk_score)
        # None scores always go last; direction applies to the value.
        jobs = sorted(jobs, key=lambda j: (
            _key(j) is None,
            -(_key(j) or 0) if sort_desc else (_key(j) or 0),
        ))
        ...
        with st.container(border=True):
            ...
            if job.status in ("READY_TO_PROPOSE", "PROPOSAL_DRAFTED"):
                ...
                if st.button("🧪 Сгенерировать кейс под вакансию", key=f"gencase_{job.id}",
                    ...
                    elif "too thin" in (cres.get("reason") or ""):
                        st.info(f"Недостаточно данных для нового кейса — отклик процитировал "
                                f"существующие релевантные кейсы {cres.get('cited_cases') or '—'}.")
                    else:
                ...
                if _jcases:
                    ...
                    for _gc in _jcases:
                        ...
                        lc1.markdown(f"- 🧪 **{_gc.title}** (кейс #{_gc.id})")
                        if lc2.button("📂 Открыть в «Кейсах»", key=f"navcase_{job.id}_{_gc.id}"):
                            st.session_state["nav_section"] = "Кейсы"
                            st.session_state["focus_case_id"] = _gc.id
                            st.rerun()
                ...
                if draft:
                    with st.expander("📝 Отклик (черновик)", expanded=False):
                        ...
                        _cost = getattr(draft, "connects_cost", None)
                        _send_label = (f"🚀 Отправить отклик за {_cost} connects" if _cost
                                       else "🚀 Отправить отклик")
                        bsub1, bsub2 = st.columns([2, 1])
                        if bsub1.button(_send_label, key=f"submit_{job.id}"):
                            ...
                            st.rerun()
                        # Read the cost without sending (dry-run), to label the button.
                        if not _cost and bsub2.button("🔎 Узнать стоимость", key=f"cost_{job.id}",
                                                       help="Открыть форму и прочитать стоимость (без отправки)"):
                            from submit import submit_proposal as _sp
                            from database import Proposal as _PP

                            def _peek():
                                _d = get_db_session()
                                try:
                                    _jb = _d.query(Job).filter(Job.id == job.id).first()
                                    _pp = _d.query(_PP).filter(_PP.job_id == job.id, _PP.status == "DRAFT").first()
                                    return _sp(_jb, _pp, _d, dry_run=True)
                                finally:
                                    _d.close()
                            with st.spinner("Читаю стоимость отправки…"):
                                ok, res = run_browser_op(_peek)
                            if ok and res.get("connects"):
                                st.success(f"Стоимость: {res['connects']} connects")
                            else:
                                st.warning((res or {}).get("reason", "не удалось прочитать"))
                            st.rerun()
            ...
            # Outcome tracking — only for SENT jobs (an outcome needs a real send).
            if job.status == "SENT":
...
def render_cases():
    ...
    # If navigated here from a job's «Открыть в Кейсах», surface that case first.
    _focus = st.session_state.pop("focus_case_id", None)
    if _focus is not None:
        cases = sorted(cases, key=lambda c: (c.id != _focus, ))
        st.info(f"Открыт кейс #{_focus} (показан первым).")
...
_SECTIONS = ["Dashboard", "Компании и задачи", "Вакансии", "Кейсы", "Клиенты", "Чат с агентом"]
# Allow other pages to navigate here (e.g. «Открыть в Кейсах»).
if st.session_state.get("nav_section") in _SECTIONS:
    st.session_state["nav_section_radio"] = st.session_state.pop("nav_section")
page = st.sidebar.radio("Разделы", _SECTIONS, key="nav_section_radio")
...
# Code version (commit + date) — so you can see exactly what's running.
if "_code_ver" not in st.session_state:
    st.session_state["_code_ver"] = code_version()
_cv = st.session_state["_code_ver"]
import database as _dbmod  # noqa: E402

try:
    _newest = max(p.stat().st_mtime for p in _APP_DIR.glob("*.py"))
    _stale = _newest > _dbmod.IMPORT_TIME + 2
except ValueError:
    _stale = False
st.sidebar.caption(f"🧩 Версия: `{_cv[0]}` · {_cv[1]}" + (" · ⚠️ устарела (перезапусти)" if _stale else ""))
...
# Persist operational UI inputs (sort, count, last section) for next launch.
_save_ui_settings()